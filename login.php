...
<head>
    ...
    <style>
        /* Тонкая доводка формы под тёмную тему (поверх глобальных переопределений styles.css) */
        ...
        html.dark .auth-divider { background-color: #334155; }
    ...
    <div class="fade-in-up w-full max-w-md rounded-2xl border border-slate-200 bg-white p-7 shadow-sm sm:p-9">
        ...
        <div class="mb-6 grid grid-cols-2 gap-2 rounded-xl bg-slate-100 p-1.5" role="tablist" aria-label="Вход или регистрация">
            <button type="button" id="tab-login" role="tab" aria-selected="true" aria-controls="form-login"
                    class="auth-tab tab-active rounded-full border border-teal-500 bg-teal-500 px-4 py-2 text-sm font-semibold text-white transition">Вход</button>
            <button type="button" id="tab-register" role="tab" aria-selected="false" aria-controls="form-register"
                    class="auth-tab rounded-full border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-600 transition hover:bg-slate-50">Регистрация</button>
        </div>
        ...
        <div class="my-6 flex items-center gap-3">
            ...
            <span class="text-xs uppercase tracking-wide text-slate-400">или по email</span>
            <div class="auth-divider h-px flex-1 bg-slate-200"></div>
        </div>
        ...
        <form id="form-login" class="space-y-4" novalidate>
            <input type="email" name="email" required autocomplete="email" placeholder="Email"
                   aria-label="Email"
                   class="auth-input w-full rounded-xl border border-slate-200 px-4 py-3 text-sm transition focus:border-teal-500 focus:outline-none focus:ring-2 focus:ring-teal-200">
            <input type="password" name="password" required autocomplete="current-password" minlength="6" placeholder="Пароль"
                   aria-label="Пароль"
                   class="auth-input w-full rounded-xl border border-slate-200 px-4 py-3 text-sm transition focus:border-teal-500 focus:outline-none focus:ring-2 focus:ring-teal-200">
        ...
        <form id="form-register" class="hidden space-y-4" novalidate>
            ...
            <input type="email" name="email" required autocomplete="email" placeholder="Email"
                   aria-label="Email"
                   class="auth-input w-full rounded-xl border border-slate-200 px-4 py-3 text-sm transition focus:border-teal-500 focus:outline-none focus:ring-2 focus:ring-teal-200">
            <input type="password" name="password" required autocomplete="new-password" minlength="6" placeholder="Пароль (минимум 6 символов)"
                   aria-label="Пароль"
                   class="auth-input w-full rounded-xl border border-slate-200 px-4 py-3 text-sm transition focus:border-teal-500 focus:outline-none focus:ring-2 focus:ring-teal-200">
        ...
        <p class="mt-6 text-center text-xs leading-relaxed text-slate-400">
            ...
            <a href="/" class="underline transition hover:text-teal-600">правила сервиса</a>
